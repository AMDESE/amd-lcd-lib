...
{
    FILE *fp;
    char tmp[LCD_MAX], val[2*LCD_MAX_CHAR];
    char file[LCD_MAX];
    int ret;
...
{
    FILE *fp;
    char line[LCD_MAX_LINE][2*LCD_MAX_CHAR];
    char tmp1[LCD_MAX_CHAR], tmp2[LCD_MAX], tmp3[LCD_MAX];
    int i=0;
    ...
    if (lcdlib_clearScreen() != 0 ) {
       printf("LCD Error: Cannot clear LCD screen %d\n", LCD_I2C_BUS);
    }
