...
static int lcdlib_clear_rows(int y, int rows)
{
    if (fb_fd < 0 || !fb_buffer)
    {
        return -1;
    }
...
static int lcdlib_fb_write_string(int x, int y, const char *str, int str_len)
{
    if (fb_fd < 0 || !fb_buffer)
    {
        return -1;
    ...
    print_err("2 Writing string to display at (%d, %d): %.*s\n", x, y, str_len, str);
    ...
    lcdlib_clear_rows(y, LINE_HEIGHT_PX);
...
int lcdlib_open_dev(void)
{
    ...
    if (fb_fd < 0)
    {
        print_err("Error: cannot open framebuffer device\n");
        return -1;
    }

    // Allocate buffer to hold one frame
    fb_buffer = (unsigned char *)malloc(OLED_WIDTH_BYTES * OLED_HEIGHT_BYTES);
    if (!fb_buffer)
    {
        print_err("Error: cannot allocate framebuffer memory\n");
        close(fb_fd);
        return -1;
    }
...
int lcdlib_write_string(LCD_msgType_t msgType, unsigned char *buffer, int str_len)
{
    ...
    if (!copied_buffer)
    {
        print_err("Error: failed to allocate memory for string copy\n");
    ...
    {
        print_err("String too long: %d, truncating to %d characters\n", str_len, max_length);
    ...
    int result = lcdlib_fb_write_string(x, y, (char *)copied_buffer, str_len);
...
int lcdlib_clearScreen(void)
{
    ...
    if (bytes_written != OLED_WIDTH_BYTES * OLED_HEIGHT_BYTES)
    {
        print_err("Error: failed to clear the framebuffer content");
        return LCD_ERR_WRITE;
    }
